...
author: "Josiah Suartono, Patricia Yin, Jiahui Liu"
date: "07/04/2021"
output:
  html_document:
    df_print: paged
...
In this study, we treat accounts 90 days past due as a “bad” account (label=1) following the standard practice of Chinese banks.
...
There are only 11 observations for "Student", none of which are label as "1"(default). Due to the low sample size of the student subset and potential confounding effects (e.g. student tend to have student loans, but also likely to receive financial support from parents other than income), we decide to remove the subset of students and instead focus on the default rate for working people and retirees.
...
There are only 31 observations for "Academic degree". We decided to merge this level with "Higher education" because they both indicate post secondary education.
...
The "Curve" refers to the ROC which plots false positive rate versus true positive rate at different probability threshold. In particular:
...
Local regression can perform poorly if the number of predictors is large because there are generally  few training samples close to an observation in a high dimension. Nearest-neighbors regression suffers from a similar problem. Because our data contains 15 predictor variables, we decide not to use local regression (e.g. loess) or nearest-neighbors method.
 
Instead, we decide to use logistic regression, SVM, and tree methods to make predictions. 
...
Using the strata argument, we want to make sure that the proportion of the response variable label=1 will be similar to that of the original data set. This is crucial as our response variable is highly imbalanced.
...
Seeing the results of VIF above, income type and occupation type seem to have multicollinearity problem, we decided to remove income type from the belief that occupation type conveys more meaningful and interpretable information.
...
We build this baseline regression model using gradient descent [6]. 
...
However, ridge regression will always generate a model involving all ten predictors. Increasing the value of $\lambda$ will tend to reduce the magnitudes of the coefficients, but will not result in exclusion of any of the variables. Hence, we conducted the lasso as a alternative to ridge regression that overcomes this disadvantage by using l1 penalty (penalizing the sum of absolute values of the coefficients).
...
#remove label from predicting variables [7]
...
It can be seen that the Lasso model predicts the highest AUC across all the logistic models, including stepwise logistic regression. This was expected as the minimum MSE on test set using cross-validation was found for Lasso model across all the models
...
[8. https://bradleyboehmke.github.io/HOML/regularized-regression.html]
  
[9. https://www.tidymodels.org/learn/work/tune-svm/]

[10. https://dzone.com/articles/variable-importance-and-how-it-is-calculated]

[11. https://www.tidymodels.org/start/tuning/]
